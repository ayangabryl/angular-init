...
export const Articles = ({ posts, featured }) => {
  ...
  const postsHeader = (
    <header className={styles.header}>
      <Heading className={styles.heading} level={5} as="h1">
        <DecoderText text="More About Me" />
      </Heading>
      <Barcode />
    </header>
  );
  ...
  const postList = (
    <div className={styles.list}>
      ...
      <SkeletonPost key={1} /> {/* Render a single skeleton */}
      {/* <SkeletonPost key={2} /> */}{' '}
      {/* Uncomment this line to render a second skeleton */}
    </div>
  );
  ...
  return (
    <article className={styles.articles}>
      <Meta title="Articles" description="" />
